...
public sealed class UIRotationWindow : UIWindow
{
    ...
    public UIRotationWindow(RotationModuleManager mgr, ActionManagerEx amex, Action openConfig) : base("Autorotation", false, new(400f, 400f), ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoFocusOnAppearing)
    {
        _mgr = mgr;
        _amex = amex;
        _subscriptions = new
        ...
        if (activeModule != null)
        {
            ...
            if (activeModule.Info?.PlanLevel > 0)
            {
                ...
                if (newSel >= 0 && _mgr.Preset != null)
                {
                    ...
                    UIMisc.HelpMarker(static () => "You have a preset activated, which fully overrides the CD plan!", FontAwesomeIcon.ExclamationTriangle);
                }
            }
        }
