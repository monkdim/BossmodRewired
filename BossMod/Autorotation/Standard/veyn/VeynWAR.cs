...
public sealed class VeynWAR(RotationModuleManager manager, Actor player) : RotationModule(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.AOE).As<AOEStrategy>("AOE", uiPriority: 90)
            .AddOption(AOEStrategy.SingleTarget, "Use single-target rotation")
            .AddOption(AOEStrategy.ForceAOE, "Use aoe rotation")
            .AddOption(AOEStrategy.Auto, "Use aoe rotation if 3+ targets would be hit, otherwise use single-target rotation; break combo if necessary")
            .AddOption(AOEStrategy.AutoFinishCombo, "Use aoe rotation if 3+ targets would be hit, otherwise use single-target rotation; finish combo route before switching");

        res.Define(Track.Burst).As<BurstStrategy>("Burst", uiPriority: 80)
            .AddOption(BurstStrategy.Automatic, "Spend gauge under potion/raidbuffs, otherwise conserve")
            .AddOption(BurstStrategy.Spend, "Spend gauge freely (as if inside burst window), ensure ST is properly maintained (useful at the end of the fight)")
            .AddOption(BurstStrategy.Conserve, "Conserve gauge as much as possible (as if outside burst window)")
            .AddOption(BurstStrategy.UnderRaidBuffs, "Spend gauge under raidbuffs, otherwise conserve; ignore potion (useful if potion is delayed)")
            .AddOption(BurstStrategy.UnderPotion, "Spend gauge under potion, otherwise conserve; ignore raidbuffs (useful for misaligned potions)")
        ...
        res.Define(Track.PrimalRend).As<PrimalRendStrategy>("PR", uiPriority: 30)
            ...
            .AddOption(PrimalRendStrategy.DelayUntilLastChance, "Delay until last possible GCD", 0, 0, ActionTargets.Hostile, 90)
        ...
        res.Define(Track.Infuriate).As<InfuriateStrategy>("Infuriate", uiPriority: 60)
            .AddOption(InfuriateStrategy.Automatic, "Try to delay uses until raidbuffs, avoiding overcap")
            .AddOption(InfuriateStrategy.Delay, "Delay, even if risking overcap")
            .AddOption(InfuriateStrategy.ForceIfNoNC, "Force unless NC active")
            .AddOption(InfuriateStrategy.ForceIfChargesCapping, "Force use if charges are about to overcap (unless NC is already active), even if it would overcap gauge")
        ...
        res.Define(Track.Onslaught).As<OnslaughtStrategy>("Onslaught", uiPriority: 20)
            .AddOption(OnslaughtStrategy.Automatic, "Always keep one charge reserved, use other charges inside burst or to prevent overcapping")
            .AddOption(OnslaughtStrategy.Forbid, "Forbid automatic use")
            .AddOption(OnslaughtStrategy.NoReserve, "Do not reserve charges: use all charges if inside burst, otherwise use as needed to prevent overcapping")
