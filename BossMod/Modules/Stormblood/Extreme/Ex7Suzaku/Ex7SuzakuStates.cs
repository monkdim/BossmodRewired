...
sealed class Ex7SuzakuStates : StateMachineBuilder
{
    ...
    private void ScreamsOfTheDamned(uint id, float delay)
    {
        ...
        ComponentCondition<RekindleP1>(id + 0x10u, 9.9f, static comp => comp.Spreads.Count != 0, "Spreadmarkers appear");
        ComponentCondition<Rout>(id + 0x20u, 3.1f, static comp => comp.NumCasts != 0, "Rect AOE")
            .DeactivateOnExit<Rout>();
        ComponentCondition<RekindleP1>(id + 0x30u, 2f, static comp => comp.Spreads.Count == 0, "Spreads resolve")
            .ActivateOnExit<FleetingSummer>()
            .DeactivateOnExit<RekindleP1>();
        ComponentCondition<FleetingSummer>(id + 0x40u, 8.9f, static comp => comp.NumCasts != 0, "Cone AOE")
    ...
    private void PhoenixDown(uint id, float delay)
    {
        ...
        ComponentCondition<RekindleP1>(id + 0x20u, 7.4f, static comp => comp.Spreads.Count != 0, "Spreadmarkers appear");
        ComponentCondition<RekindleP1>(id + 0x30u, 5.1f, static comp => comp.Spreads.Count == 0, "Spreads resolve")
            .DeactivateOnExit<RekindleP1>();
        Cast(id + 0x40u, AID.Cremate, 7.9f, 10.9f, "Tankbuster")
            .SetHint(StateMachine.StateHint.Tankbuster);
        ComponentCondition<ScarletPlumeTailFeather>(id + 0x50u, 1.1f, static comp => comp.AOEs.Count == 0, "Feathers resolve")
    ...
    private void LimitBreakPhase(uint id, float delay)
    {
        ...
        ComponentCondition<RapturousEchoTowers>(id + 0x10u, 20.4f, static comp => comp.Towers.Count != 0, "DDR start");
        ComponentCondition<RapturousEchoTowers>(id + 0x20u, 31.2f, static comp => comp.Towers.Count == 0, "DDR end")
        ...
        Condition(id + 0x40u, 0.7f, () => Module.Arena.Bounds.GetVerticeCount() == 160, "Donut arena")
    ...
    private void Phase2Start(uint id, float delay)
    {
        ...
        ComponentCondition<MesmerizingMelody>(id + 0x10u, 11.1f, static comp => comp.NumCasts != 0, "Pull")
    ...
    {
        ComponentCondition<Hotspot>(id, delay, static comp => comp.AOEs.Count == 16, "Panels");
        float[] delays = [11.7f, 1.25f, 1.25f, 1.25f, 1.25f, 1.25f, 1.25f, 1.25f, 10.7f, 1.25f, 1.25f, 1.25f, 1.25f, 1.25f, 1.25f, 1.25f];
        for (var i = 0u; i < 16u; ++i)
        {
            var casts = i + 1u;
            var condition = ComponentCondition<Hotspot>(id + casts * 0x10u, delays[i], comp => comp.NumCasts == casts, $"Hotspot {casts}");
            if (i == 15u)
            {
    ...
    private void Hotspot2(uint id, float delay)
    {
        ...
        {
            var casts = i + 1u;
            ComponentCondition<Hotspot>(id + 0x10u + casts * 0x10u, delays[i], comp => comp.NumCasts == casts, $"Hotspot {casts}");
        }
        ...
        {
            var casts = i + 1u;
            var condition = ComponentCondition<Hotspot>(id + 0xA0u + (casts - 8u) * 0x10u, delays[i], comp => comp.NumCasts == casts, $"Hotspot {casts}");
            if (i == 15u)
            {
                condition.DeactivateOnExit<Hotspot>();
            }
    ...
    {
        ComponentCondition<Hotspot>(id, delay, static comp => comp.AOEs.Count == 8, "Panels")
            .ActivateOnEnter<RekindleP2>();
        ComponentCondition<RekindleP2>(id + 0x10u, 2.9f, static comp => comp.Spreads.Count != 0, "Spreadmarkers appear")
            .ActivateOnExit<WellOfFlame>();
        ComponentCondition<WellOfFlame>(id + 0x20u, 4.1f, static comp => comp.NumCasts != 0, "Frontal AOE")
            .DeactivateOnExit<WellOfFlame>();
        ComponentCondition<RekindleP2>(id + 0x30u, 1f, static comp => comp.Spreads.Count == 0, "Spreads resolve")
        ...
        {
            var casts = i + 1u;
            var condition = ComponentCondition<Hotspot>(id + 0x30u + casts * 0x10u, delays[i], comp => comp.NumCasts == casts, $"Hotspot {casts}");
        }
        ComponentCondition<PayThePiperHotspotCombo>(id + 0x80u, 5f, static comp => comp.State.Count != 0, "Forced march tethers");
        ComponentCondition<PayThePiperHotspotCombo>(id + 0x90u, 10.3f, static comp => comp.NumActiveForcedMarches != 0, "Forced march starts");
        for (var i = 4u; i < 8u; ++i)
        {
            var casts = i + 1u;
            ComponentCondition<Hotspot>(id + 0x90u + (casts - 4u) * 0x10u, delays[i], comp => comp.NumCasts == casts, $"Hotspot {casts}");
            if (i == 6u)
            {
                ComponentCondition<PayThePiperHotspotCombo>(id + 0xC1u, 1f, static comp => comp.NumActiveForcedMarches == 0, "Forced march ends");
            }
        }
        ...
        {
            var casts = i + 1u;
            ComponentCondition<Hotspot>(id + 0xE0u + (casts - 8u) * 0x10u, delays[i], comp => comp.NumCasts + 8 == casts, $"Hotspot {casts}");
        }
        PhantomFlurry(id + 0x130u, 3.2f);
        for (var i = 12u; i < 16u; ++i)
        {
            var casts = i + 1u;
            var condition = ComponentCondition<Hotspot>(id + 0x160u + (casts - 12u) * 0x10u, delays[i], comp => comp.NumCasts + 8 == casts, $"Hotspot {casts}");
            if (i == 15u)
            {
                condition.DeactivateOnExit<Hotspot>();
            }
    ...
    private void Phase2AfterHotspot1(uint id, float delay)
    {
        ...
        ComponentCondition<PayThePiperRegular>(id + 0x20u, 2.5f, static comp => comp.State.Count != 0, "Forced march tethers");
        ComponentCondition<PayThePiperRegular>(id + 0x30u, 10.3f, static comp => comp.NumActiveForcedMarches != 0, "Forced march starts");
        ComponentCondition<PayThePiperRegular>(id + 0x40u, 4f, static comp => comp.NumActiveForcedMarches == 0, "Forced march ends")
    ...
    private void Phase2AfterHotspot2(uint id, float delay)
    {
        ...
        SouthronStar(id + 0x40u, 5.1f)
            ...
            .ActivateOnExit<RekindleP2>();
        ComponentCondition<IncandescentInterlude>(id + 0x50u, 14.3f, static comp => comp.TowerCache.Count != 0, "Towers appear");
        ComponentCondition<RekindleP2>(id + 0x60u, 4.8f, static comp => comp.Spreads.Count != 0, "Spreadmarkers appear");
        ComponentCondition<RuthlessRefrain>(id + 0x70u, 2.5f, static comp => comp.NumCasts != 0, "Knockback")
            .SetHint(StateMachine.StateHint.Knockback)
            .DeactivateOnExit<RuthlessRefrain>();
        ComponentCondition<IncandescentInterlude>(id + 0x80u, 2.3f, static comp => comp.Towers.Count == 0, "Towers resolve")
            .DeactivateOnExit<IncandescentInterlude>();
        ComponentCondition<RekindleP2>(id + 0x90u, 0.25f, static comp => comp.Spreads.Count == 0, "Spreads resolve")
            .ActivateOnExit<WellOfFlame>()
            .DeactivateOnExit<RekindleP2>();
        ComponentCondition<WellOfFlame>(id + 0xA0u, 4.8f, static comp => comp.NumCasts != 0, "Frontal AOE")
            .DeactivateOnExit<WellOfFlame>()
    ...
    private State PhantomFlurry(uint id, float delay, float delay2 = 1.6f)
    {
        ...
        ComponentCondition<PhantomFlurryTB>(id + 0x10u, 0.1f, static comp => comp.NumCasts == 1, "Tank swap")
            .SetHint(StateMachine.StateHint.Tankbuster);
        ComponentCondition<PhantomFlurryAOE>(id + 0x20u, 6.1f, static comp => comp.NumCasts != 0, "Frontal AOE")
            .DeactivateOnExit<PhantomFlurryAOE>();
        return ComponentCondition<PhantomFlurryTB>(id + 0x30u, delay2, static comp => comp.NumCasts == 2)
    ...
    {
        ComponentCondition<RekindleP2>(id, delay, static comp => comp.Spreads.Count != 0, "Spreadmarkers appear")
            .ActivateOnExit<WellOfFlame>();
        ComponentCondition<WellOfFlame>(id + 0x10u, 4.2f, static comp => comp.NumCasts != 0, "Frontal AOE")
            .DeactivateOnExit<WellOfFlame>();
        ComponentCondition<RekindleP2>(id + 0x20u, 0.9f, static comp => comp.Spreads.Count == 0, "Spreads resolve")
            .DeactivateOnExit<RekindleP2>()
            .ActivateOnExit<ScathingNet>();
        ComponentCondition<ScathingNet>(id + 0x30u, 1.2f, static comp => comp.Stacks.Count != 0, "Stackmarker appears");
        return ComponentCondition<ScathingNet>(id + 0x40u, 5.1f, static comp => comp.Stacks.Count == 0, "Stack resolves")
