...
sealed class CLL2AlbeleoTheMaleficentStates : StateMachineBuilder
{
    public CLL2AlbeleoTheMaleficentStates(BossModule module) : base(module)
    {
        TrivialPhase()
            ...
            .Raw.Update = () => module.PrimaryActor.IsDeadOrDestroyed || module.PrimaryActor.HPMP.CurHP == 1u;
    }
}

[ModuleInfo(BossModuleInfo.Maturity.Verified, Contributors = "The Combat Reborn Team (Malediktus)", GroupType = BossModuleInfo.GroupType.CastrumLacusLitore, GroupID = 735u, NameID = 9433u)]
public sealed class CLL2AlbeleoTheMaleficent : BossModule
{
    public CLL2AlbeleoTheMaleficent(WorldState ws, Actor primary) : this(ws, primary, BuildArena()) { }

    private CLL2AlbeleoTheMaleficent(WorldState ws, Actor primary, (WPos center, ArenaBoundsCustom arena) a) : base(ws, primary, a.center, a.arena) { }

    private static (WPos center, ArenaBoundsCustom arena) BuildArena()
    {
        WPos[] vertices = [new(87.45f, -409.99f), new(87.45f, -408.35f), new(87.75f, -407.95f),
        ...
        var arena = new ArenaBoundsCustom([new PolygonCustom(vertices)]) { Y = 230.5f, BorderY = 230.5f };
        return (arena.Center, arena);
    }

    private readonly uint[] adds = [(uint)OID.AlbeleosCanisDirus, (uint)OID.AlbeleosHrodvitnir, (uint)OID.AlbeleosMonstrosity, (uint)OID.FourthLegionColossus,
