...
sealed class Ex4IfritStates : StateMachineBuilder
{
    ...
    {
        NailsSubphase<Ex4IfritAINails1, Ex4IfritAIHellfire1>(id, "4 nails at ~85%", false, false, 45f);
        NailsSubphase<Ex4IfritAINails2, Ex4IfritAIHellfire2>(id + 0x10000u, "8 nails at ~50%", true, true, 75f);
        NailsSubphase<Ex4IfritAINails3, Ex4IfritAIHellfire3>(id + 0x20000u, "13 nails at ~30%", true, false, 115f);
        SimpleState(id + 0x30000u, 1000f, "Enrage")
    ...
    {
        Condition(id, 1000u, () => _module.SmallNails.Any(a => a.IsTargetable && !a.IsDead), name)
            .ActivateOnEnter<Incinerate>()
            .ActivateOnEnter<Eruption>()
            ...
            .DeactivateOnExit<Ex4IfritAINormal>();
        Condition(id + 1u, nailEnrage, () => !Module.PrimaryActor.IsTargetable, "Nails enrage", 1000f) // note: not using invincibility as a condition here, since boss can become invincible early during nails if brought below 10%
            .ActivateOnEnter<Incinerate>()
        ...
        CastStart(id + 0x10u, AID.Hellfire, 2.7f)
            .ActivateOnEnter<Hellfire>()
            .ActivateOnEnter<AIHellfire>();
        CastEnd(id + 0x11u, 2f, "Raidwide")
            .DeactivateOnExit<Hellfire>()
            .SetHint(StateMachine.StateHint.Raidwide);
        Condition(id + 0x20u, 5.1f, () => Module.PrimaryActor.FindStatus((uint)SID.Invincibility) == null, "Invincible end", 10)
            .DeactivateOnExit<AIHellfire>()
            .SetHint(StateMachine.StateHint.DowntimeEnd);
        CastStart(id + 0x30u, AID.RadiantPlume, 7.3f) // TODO: do we even need any AI here?..
            .ActivateOnEnter<Incinerate>()
            .DeactivateOnExit<Incinerate>(); // allow dodging plumes near mt
        CastEnd(id + 0x31u, 2.2f)
            .ActivateOnEnter<RadiantPlume>();
        ComponentCondition<RadiantPlume>(id + 0x32, 0.9f, static comp => comp.Casters.Count == 0, "Plumes")
            .DeactivateOnExit<RadiantPlume>();

        // note: cyclones could be skipped with high enough dps
        Condition(id + 0x40u, 13.5f, () => !Module.PrimaryActor.IsTargetable || _module.SmallNails.Any(a => a.IsTargetable && !a.IsDead), "Disappear", 10)
            .ActivateOnEnter<Incinerate>()
            .ActivateOnEnter<Ex4IfritAINormal>()
            .DeactivateOnExit<Incinerate>()
            .DeactivateOnExit<Ex4IfritAINormal>();
        Targetable(id + 0x50u, true, 15.4f, "Cyclones + Reappear")
