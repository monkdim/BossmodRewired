...
sealed class A10RhalgrEmissaryStates : StateMachineBuilder
{
    ...
    {
        ComponentCondition<DestructiveCharge>(id, delay, static comp => comp.AOEs.Count != 0);
        CastStart(id + 0x10u, AID.LightningBolt, 6.7f);
        ComponentCondition<DestructiveCharge>(id + 0x20u, 2.4f, static comp => comp.NumCasts != 0, "Diagonal cleaves")
            .ExecOnExit<DestructiveCharge>(static comp => comp.NumCasts = 0);
        CastEnd(id + 0x30u, 0.6f);
        ComponentCondition<LightningBolt>(id + 0x40u, 1f, static comp => comp.Casters.Count != 0); // 3x3, 3sec cast each, 2s between sets
        ComponentCondition<LightningBolt>(id + 0x50u, 7f, static comp => comp.Casters.Count == 0, "Puddles resolve");
    }
    ...
    private void DestructiveChargeDestructiveStatic(uint id, float delay)
    {
        ...
        ComponentCondition<DestructiveCharge>(id + 0x20u, 5.2f, static comp => comp.NumCasts != 0, "Diagonal cleaves")
            .ExecOnExit<DestructiveCharge>(static comp => comp.NumCasts = 0);
    ...
    private void BoltsFromTheBlue(uint id, float delay)
    {
        ...
        ComponentCondition<BoltsFromTheBlue>(id + 2u, 1f, static comp => comp.NumCasts != 0, "Raidwide")
            .ExecOnExit<BoltsFromTheBlue>(static comp => comp.NumCasts = 0)
    ...
    private void Boltloop(uint id, float delay)
    {
        ...
        ComponentCondition<Boltloop>(id + 0x10u, 1.1f, static comp => comp.NumCasts >= 2, "Concentric AOE 1");
        ComponentCondition<Boltloop>(id + 0x20u, 2f, static comp => comp.NumCasts >= 4, "Concentric AOE 2");
        ComponentCondition<Boltloop>(id + 0x30u, 2f, static comp => comp.NumCasts >= 6, "Concentric AOE 3")
            .ExecOnExit<Boltloop>(static comp => comp.NumCasts = 0);
    }
