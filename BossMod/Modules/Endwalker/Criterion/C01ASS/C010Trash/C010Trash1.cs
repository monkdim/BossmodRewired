...
[ModuleInfo(BossModuleInfo.Maturity.Verified, Contributors = "Malediktus", PrimaryActorOID = (uint)OID.NUdumbara, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 878u, NameID = 11511u, SortOrder = 1)]
public sealed class C010NTrash1(WorldState ws, Actor primary) : Trash1Arena(ws, primary, false);

[ModuleInfo(BossModuleInfo.Maturity.Verified, Contributors = "Malediktus", PrimaryActorOID = (uint)OID.SUdumbara, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 879u, NameID = 11511u, SortOrder = 1)]
public sealed class C010STrash1(WorldState ws, Actor primary) : Trash1Arena(ws, primary, true);

public abstract class Trash1Arena : BossModule
{
    public Trash1Arena(WorldState ws, Actor primary, bool savage) : this(ws, primary, BuildArena(), savage) { }

    private Trash1Arena(WorldState ws, Actor primary, (WPos center, ArenaBoundsCustom arena) a, bool savage) : base(ws, primary, a.center, a.arena, savage)
    {
        _savage = savage;
    }
    ...
    {
        WPos[] verticesExterior = [new(-338.64f, -114.93f), new(-331.43f, -114.84f), new(-330.79f, -114.66f), new(-330.34f, -114.12f), new(-330.20f, -113.44f),
            ...
            new(-338.64f, -114.93f)];
        WPos[] verticesHole1 = [new(-297.76f, -91.76f), new(-297.22f, -91.67f), new(-295.75f, -89.65f), new(-295.95f, -86.31f), new(-294.77f, -85.82f),
        ...
        WPos[] verticesHole2 = [new(-377.26f, -93.01f), new(-376.64f, -92.84f), new(-375.96f, -92.78f), new(-375.36f, -92.50f), new(-372.55f, -91.71f),
            ...
            new(-377.52f, -93.02f)];
        var arena = new ArenaBoundsCustom([new PolygonCustom(verticesExterior)], [new PolygonCustom(verticesHole1), new PolygonCustom(verticesHole2)]);
